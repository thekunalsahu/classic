...
class _DashboardScreenState extends State<DashboardScreen> with TickerProviderStateMixin {
  ...
  Future<void> _runScan() async {
    ...
    try {
      ...
      setState(() {
        ...
        _tasksList.insert(0, {"title": "Violation Detected", "desc": "Unauthorized structure in Sector ${query.toUpperCase()}", "status": "Critical", "time": DateFormat('HH:mm a').format(DateTime.now())});
      });
      _speak("Alert. Unauthorized encroachment detected.");
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      return Scaffold(
        ...
        drawer: isMobile ? Drawer(child: _sidebar()) : null,
        body: Row(children: [
          if (!isMobile) _sidebar(),
          Expanded(child: Column(children: [_topNav(isMobile), Expanded(child: _buildMainContent(isMobile))]))
        ]),
        floatingActionButton: FloatingActionButton(onPressed: _showChatbot, backgroundColor: Colors.cyanAccent, child: const Icon(Icons.auto_awesome, color: Colors.black)),
      );
    });
  }
  ...
  Widget _buildMainContent(bool isMobile) {
    ...
    if (isMobile) return ListView(padding: const EdgeInsets.all(12), children: [SizedBox(height: 350, child: _mapView(isMobile)), const SizedBox(height: 12), _rightPanel(isMobile)]);
  ...
  Widget _mapView(bool isMobile) {
    return ClipRRect(borderRadius: BorderRadius.circular(12), child: Stack(children: [
      FlutterMap(mapController: _mapCtrl, options: MapOptions(initialCenter: _loc, initialZoom: 18.0), children: [
        TileLayer(urlTemplate: 'https://mt1.google.com/vt/lyrs=y&x={x}&y={y}&z={z}', subdomains: const ['mt0', 'mt1', 'mt2', 'mt3']),
      ...
      Positioned(top: 15, right: 15, child: Container(width: isMobile ? 250 : 350, decoration: BoxDecoration(color: const Color(0xFF0B1221).withOpacity(0.9), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.white10)), child: TextField(controller: _searchCtrl, style: const TextStyle(color: Colors.white), decoration: const InputDecoration(hintText: "Search Location...", border: InputBorder.none, prefixIcon: Icon(Icons.search, color: Colors.cyanAccent)), onSubmitted: (_) => _runScan()))),
  ...
  Widget _sideBtn(IconData i, String l, bool a, {Color? color, VoidCallback? tap}) => InkWell(onTap: tap, child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 15), decoration: BoxDecoration(border: a ? const Border(left: BorderSide(color: Colors.cyanAccent, width: 4)) : null), child: Column(children: [Icon(i, color: a ? Colors.cyanAccent : (color ?? Colors.white54)), Text(l, style: TextStyle(color: a ? Colors.cyanAccent : (color ?? Colors.white54), fontSize: 10))])));
  ...
  Widget _topNav(bool isMobile) => Container(height: 60, padding: const EdgeInsets.symmetric(horizontal: 20), decoration: const BoxDecoration(color: Color(0xFF0B1221), border: Border(bottom: BorderSide(color: Colors.white10))), child: Row(children: [
    ...
    const SizedBox(width: 10), const Text("Gravity AI", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
    const Spacer(), IconButton(icon: const Icon(Icons.notifications_none, color: Colors.white), onPressed: _showNotificationPanel)
  ]));
  ...
  Widget _rightPanel(bool isMobile) => Container(padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: const Color(0xFF0B1221), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.white10)), child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    if (widget.isOfficer) ...[
      ...
      _actionBtn("Field Evidence", Icons.camera_alt, _captureEvidence),
      const Divider(color: Colors.white10, height: 30),
    ],
    if (!_ready) const Center(child: Text("Waiting for scan...", style: TextStyle(color: Colors.white30)))
    else ...[
      _stat("Encroached Area", "$_area m²", Colors.white),
      _stat("Risk Score", "$_risk/100", Colors.orangeAccent),
      const SizedBox(height: 10),
      if (widget.isOfficer) ...[
        Row(children: [Expanded(child: _btn("Compare", Icons.compare, _showComp)), const SizedBox(width: 10), Expanded(child: _btn("PDF", Icons.picture_as_pdf, _makePDF))]),
        const SizedBox(height: 10),
        _actionBtn("Generate AI Notice", Icons.auto_awesome, _showNotice),
      ]
    ]
  ])));

  Widget _stat(String t, String v, Color c) => Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white.withOpacity(0.05), borderRadius: BorderRadius.circular(8)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(t, style: const TextStyle(color: Colors.white54, fontSize: 10)), Text(v, style: TextStyle(color: c, fontSize: 18, fontWeight: FontWeight.bold))]));
  Widget _btn(String t, IconData i, VoidCallback tap) => ElevatedButton.icon(onPressed: tap, icon: Icon(i, size: 14), label: Text(t, style: const TextStyle(fontSize: 11)), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1E293B)));
  Widget _actionBtn(String t, IconData i, VoidCallback tap) => Padding(padding: const EdgeInsets.only(bottom: 8), child: SizedBox(width: double.infinity, child: ElevatedButton.icon(onPressed: tap, icon: Icon(i, size: 16), label: Text(t), style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF1E293B), alignment: Alignment.centerLeft))));
  ...
  void _showNotice() async {
    ...
    String noticeText = "Generating Notice...";
    try {
      final res = await http.post(Uri.parse("https://api.groq.com/openai/v1/chat/completions"), headers: {"Authorization": "Bearer $kGroqKey", "Content-Type": "application/json"}, body: jsonEncode({"model": "llama-3.3-70b-versatile", "messages": [{"role": "system", "content": "Draft a strict government eviction notice."}, {"role": "user", "content": "Area: $_area sqm, Location: ${_searchCtrl.text}"}]}));
      if (res.statusCode == 200) noticeText = jsonDecode(res.body)['choices'][0]['message']['content'];
    } catch (_) {}
    _notice = noticeText;
    if (mounted) Navigator.pop(context);
    showDialog(context: context, builder: (c) => AlertDialog(backgroundColor: const Color(0xFF0B1221), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: const BorderSide(color: Colors.cyanAccent)), title: const Text("AI NOTICE", style: TextStyle(color: Colors.cyanAccent)), content: SingleChildScrollView(child: Text(noticeText, style: const TextStyle(color: Colors.white70))), actions: [TextButton(onPressed: () => Navigator.pop(c), child: const Text("CLOSE"))]));
  }

  void _showNotificationPanel() { showGeneralDialog(context: context, pageBuilder: (c, a1, a2) => Align(alignment: Alignment.topRight, child: Container(width: 300, margin: const EdgeInsets.only(top: 60, right: 20), decoration: BoxDecoration(color: const Color(0xFF0F172A), borderRadius: BorderRadius.circular(12)), child: const Material(color: Colors.transparent, child: Padding(padding: EdgeInsets.all(20), child: Text("No new alerts.", style: TextStyle(color: Colors.white54))))))); }

  void _showComp() { showDialog(context: context, builder: (c) => Dialog(backgroundColor: const Color(0xFF0B1221), child: Padding(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, children: [const Text("COMPARISON VIEW", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)), const SizedBox(height: 20), Row(children: [Expanded(child: Container(height: 200, color: Colors.white10, child: const Center(child: Text("2021", style: TextStyle(color: Colors.white30))))), const SizedBox(width: 10), Expanded(child: Container(height: 200, color: Colors.white10, child: const Center(child: Text("2024", style: TextStyle(color: Colors.white30)))))])])))); }
  ...
  void _showChatbot() {
    showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (c) => Container(height: MediaQuery.of(context).size.height * 0.7, decoration: const BoxDecoration(color: Color(0xFF0F172A), borderRadius: BorderRadius.vertical(top: Radius.circular(20))), child: Column(children: [
      const Padding(padding: EdgeInsets.all(20), child: Text("Gravity Assistant", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))),
      Expanded(child: ListView.builder(itemCount: _chatMsgs.length, padding: const EdgeInsets.all(15), itemBuilder: (c, i) => Text(_chatMsgs[i]["text"]!, style: const TextStyle(color: Colors.white70)))),
      Padding(padding: const EdgeInsets.all(20), child: TextField(controller: _chatCtrl, style: const TextStyle(color: Colors.white), onSubmitted: (v) => _getGroqResponse(v, (fn)=>setState(fn))))
    ])));
  }

  Future<void> _getGroqResponse(String q, Function setM) async {
    _chatMsgs.add({"role": "user", "text": q}); _chatCtrl.clear();
    try {
      final res = await http.post(Uri.parse("https://api.groq.com/openai/v1/chat/completions"), headers: {"Authorization": "Bearer $kGroqKey", "Content-Type": "application/json"}, body: jsonEncode({"model": "llama-3.3-70b-versatile", "messages": [{"role": "system", "content": "Assistant mode."}, {"role": "user", "content": q}]}));
      if (res.statusCode == 200) _chatMsgs.add({"role": "ai", "text": jsonDecode(res.body)['choices'][0]['message']['content']});
    } catch (_) {}
    setState(() {});
  }

  void _showBhuPrahari() { showDialog(context: context, builder: (c) => AlertDialog(backgroundColor: const Color(0xFF0F172A), title: const Text("Citizen Portal", style: TextStyle(color: Colors.white)), content: const Text("Community reporting system active.", style: TextStyle(color: Colors.white70)))); }
}
